...
pbp_2019_original <-
  read_csv(
    url(
      "https://github.com/ryurko/nflscrapR-data/raw/master/play_by_play_data/regular_season/reg_pbp_2019.csv"
    )
  )
...
pass_over_run <- pbp2 %>%
  filter(wp >= .20 &
           wp <= .80 &
           down <= 2 & qtr <= 2 & half_seconds_remaining > 120) %>%
  group_by(posteam) %>%
...
pass_over_run_plot <- pass_over_run %>%
  ggplot(aes(
    x = reorder(posteam, -mean_pass),
    y = mean_pass,
    label = posteam
  )) + geom_point() + scale_y_continuous(labels = scales::percent) + geom_text(vjust = 0, nudge_y = 0.003) + labs(x = "Teams", y = "Average Pass Play")
